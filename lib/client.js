...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// --- token rates --------------------------------------------------
		// V4.1 Flash, effective 2026-09-10 12:00 Beijing: per 1M tokens, peak is
		// exactly 2× off-peak, weekends are off-peak all day. CNY and USD are
		// both published officially (the English notice lists USD) — they are
		// NOT inter-converted, so each table below is verbatim.
		const RATES_CNY = {
		...
		// V4 Pro keeps its own (unchanged) prices until 12:00 Beijing on
		// 2026-09-14 = 04:00 UTC, when Pro requests are routed to V4.1 Flash and
		// billed at Flash rates. Until then the card carries a short note.
		...
		/** "0.02" / "1" / "0.003"-style number: fixed decimals, trailing zeros trimmed. */
		function formatAmount(value, decimals) {
			return value.toFixed(decimals).replace(/\.?0+$/, "");
		}

		/** Whether V4 Pro is still billed separately (before the routing date). */
		function proBilledSeparately(now) {
			return now.getTime() < PRO_ROUTED_FROM;
		}

		/** Billing rows from the active currency table (CNY ≤2 decimals, USD 3). */
		function rateRows(currency) {
			const table = currency === "usd" ? RATES_USD : RATES_CNY;
			const decimals = currency === "usd" ? 3 : 2;
			return COST_ROWS.map((row) => ({
				...row,
				off: table[row.key].off,
				peak: table[row.key].peak,
				decimals
			}));
		}

		/** Token-cost card: three billing rows, off-peak vs peak, with the
		 * currently active column emphasized and a 元/$ switch. */
		function CostCard({ peak, now }) {
			const [currency, setCurrency] = react.useState("cny");
			const rows = rateRows(currency);
			const column = peak ? "peak" : "off";
			const unit = currency === "usd" ? "$" : "元";
			return react.createElement("div", {
				"data-deepseek-cost": true,
				className: "dpk-cost"
			}, [
				react.createElement("div", { className: "dpk-cost-head", key: "head" }, [
					react.createElement("span", { className: "dpk-cost-title" }, "Token cost · V4.1 Flash"),
					react.createElement("div", { className: "dpk-cost-toggle", role: "group", "aria-label": "Rate currency" }, [
						react.createElement("button", {
							type: "button",
							className: currency === "cny" ? "dpk-seg dpk-seg-on" : "dpk-seg",
							"aria-pressed": currency === "cny",
							"aria-label": "Chinese yuan rates",
							onClick: (event) => {
						...
						react.createElement("button", {
							type: "button",
							className: currency === "usd" ? "dpk-seg dpk-seg-on" : "dpk-seg",
							"aria-pressed": currency === "usd",
							"aria-label": "US dollar rates",
							onClick: (event) => {
								event.stopPropagation();
								setCurrency("usd");
							}
				...
				react.createElement("div", { className: "dpk-cost-cols", key: "cols" }, [
					react.createElement("span", { className: "dpk-cost-unit" }, `per 1M tokens (${unit})`),
					react.createElement("span", { className: "dpk-cost-colhead" }, "off-peak"),
					react.createElement("span", { className: "dpk-cost-colhead" }, "peak")
				]),
				rows.map((row) => react.createElement("div", {
					key: row.key,
					className: "dpk-cost-row"
				}, [
					react.createElement("span", { className: "dpk-cost-label", title: row.label }, row.label),
					react.createElement("span", { className: column === "off" ? "dpk-cost-cell dpk-cost-now" : "dpk-cost-cell" }, formatAmount(row.off, row.decimals)),
					react.createElement("span", { className: column === "peak" ? "dpk-cost-cell dpk-cost-now" : "dpk-cost-cell" }, formatAmount(row.peak, row.decimals))
				])),
				// Auto-expiring: hidden once Pro is routed to Flash (Sep 14 04:00 UTC).
				proBilledSeparately(now)
					? react.createElement("div", {
							key: "pro",
							className: "dpk-cost-note",
							"data-deepseek-pro-note": true
						}, "V4 Pro billed separately until Sep 14")
					: null
			]);
		}
		...
		const STYLES = [
			...
			".dpk-dot-peak{background:var(--dpk-peak,#d4576b);box-shadow:0 0 0 3px color-mix(in srgb,var(--dpk-peak) 22%,transparent)}",
			".dpk-dot-offpeak{background:var(--dpk-offpeak,#2aa876);box-shadow:0 0 0 3px color-mix(in srgb,var(--dpk-offpeak) 22%,transparent)}",
			".dpk-btn:hover .dpk-dot-peak{box-shadow:0 0 0 5px color-mix(in srgb,var(--dpk-peak) 32%,transparent)}",
			".dpk-btn:hover .dpk-dot-offpeak{box-shadow:0 0 0 5px color-mix(in srgb,var(--dpk-offpeak) 32%,transparent)}",
			...
			".dpk-panel{position:fixed;width:300px;max-width:calc(100vw - 16px);box-sizing:border-box;z-index:1000;border-radius:18px;border:1px solid var(--dsw-alias-border-l2,rgba(0,0,0,.12));color:var(--dsw-alias-label-primary,#111);box-shadow:0 18px 48px -14px rgba(0,0,0,.26),0 4px 12px -4px rgba(0,0,0,.12);outline:none;display:flex;flex-direction:column;font:var(--dsw-font-xs-13,13px/1.5 system-ui,sans-serif);transition:background-color 200ms ease-out;--dpk-peak:#d4576b;--dpk-offpeak:#2aa876;--dpk-peak-text:#b1374b;--dpk-offpeak-text:#0c7a52;--dpk-tint:var(--dpk-offpeak);--dpk-panel-bg:color-mix(in srgb,var(--dpk-tint) 5%,var(--dsw-alias-bg-layer-2,#fff));background:var(--dpk-panel-bg)}",
			".dpk-panel.dpk-peak{--dpk-tint:var(--dpk-peak)}",
			"body[data-ds-dark-theme] .dpk-panel{--dpk-peak:#b9565f;--dpk-offpeak:#2a8f68;--dpk-peak-text:#f0818c;--dpk-offpeak-text:#33cf8d;--dpk-panel-bg:color-mix(in srgb,var(--dpk-tint) 6%,var(--dsw-alias-bg-layer-2,#000))}",
			...
			".dpk-eyebrow{color:var(--dsw-alias-label-secondary,#555);font-size:10px;line-height:14px;font-weight:600;text-transform:uppercase;letter-spacing:.08em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}",
			...
			".dpk-peak .dpk-pill{color:var(--dpk-peak-text,#b1374b);background:color-mix(in srgb,var(--dpk-peak) 11%,transparent)}",
			".dpk-offpeak .dpk-pill{color:var(--dpk-offpeak-text,#0c7a52);background:color-mix(in srgb,var(--dpk-offpeak) 11%,transparent)}",
			...
			".dpk-hero-card{display:flex;flex-direction:column;gap:2px;background:linear-gradient(180deg,var(--dsw-alias-bg-layer-1,#f8f8f8),color-mix(in srgb,var(--dsw-alias-bg-layer-1,#f8f8f8) 90%,var(--dsw-alias-bg-layer-2,#fff)));border:1px solid var(--dsw-alias-border-l1,rgba(0,0,0,.04));border-radius:12px;padding:9px 12px;box-shadow:0 1px 2px rgba(0,0,0,.03)}",
			".dpk-hero-label{color:var(--dsw-alias-label-secondary,#555);font-size:10px;line-height:14px;font-weight:600;text-transform:uppercase;letter-spacing:.08em}",
			".dpk-hero-numeral{font-family:var(--dsw-font-family,system-ui,sans-serif);font-size:34px;line-height:40px;font-weight:280;font-variant-numeric:tabular-nums;letter-spacing:-.03em}",
			...
			".dpk-cost{padding:12px 16px 0;display:flex;flex-direction:column;gap:2px}",
			".dpk-cost-head{display:flex;align-items:center;justify-content:space-between;gap:10px}",
			".dpk-cost-title{color:var(--dsw-alias-label-secondary,#555);font-size:10px;line-height:14px;font-weight:600;text-transform:uppercase;letter-spacing:.08em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}",
			".dpk-cost-toggle{display:inline-flex;flex:none;gap:1px;padding:1px;border:1px solid var(--dsw-alias-border-l1,rgba(0,0,0,.06));border-radius:7px;background:var(--dsw-alias-bg-layer-1,#f2f2f2)}",
			".dpk-seg{display:inline-flex;align-items:center;justify-content:center;min-width:24px;height:16px;padding:0 6px;border:none;border-radius:5px;background:transparent;color:var(--dsw-alias-label-tertiary,#777);font-size:11px;line-height:1;font-weight:600;cursor:pointer;transition:background-color 120ms ease-out,color 120ms ease-out}",
			".dpk-seg:hover{color:var(--dsw-alias-label-primary,#111)}",
			".dpk-seg-on{background:var(--dsw-alias-bg-layer-3,#fff);color:var(--dsw-alias-label-primary,#111);box-shadow:0 1px 2px rgba(0,0,0,.12)}",
			".dpk-seg:focus-visible{outline:2px solid var(--dsw-alias-state-business-primary,#4176e6);outline-offset:1px}",
			".dpk-cost-cols{display:grid;grid-template-columns:minmax(0,1fr) 64px 64px;gap:0 8px;margin-top:3px}",
			".dpk-cost-unit{color:var(--dsw-alias-label-tertiary,#888);font-size:9px;line-height:12px;text-transform:uppercase;letter-spacing:.06em;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}",
			".dpk-cost-colhead{text-align:right;color:var(--dsw-alias-label-tertiary,#888);font-size:9px;line-height:12px;text-transform:uppercase;letter-spacing:.06em;font-weight:600}",
			".dpk-cost-row{display:grid;grid-template-columns:minmax(0,1fr) 64px 64px;gap:0 8px;align-items:baseline;min-width:0}",
			".dpk-cost-label{color:var(--dsw-alias-label-secondary,#555);font-size:11px;line-height:18px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}",
			".dpk-cost-cell{text-align:right;color:var(--dsw-alias-label-tertiary,#999);font-size:12px;line-height:18px;font-weight:500;font-variant-numeric:tabular-nums;transition:color 150ms ease-out}",
			".dpk-cost-now{font-weight:700}",
			".dpk-cost-note{margin-top:3px;color:var(--dsw-alias-label-secondary,#555);font-size:9.5px;line-height:13px}",
			".dpk-peak .dpk-cost-now{color:var(--dpk-peak-text,#b1374b)}",
			".dpk-offpeak .dpk-cost-now{color:var(--dpk-offpeak-text,#0c7a52)}",
		...
		function Dashboard({ peak, now, onClose, caretLeft }) {
			...
			return [
				...
				react.createElement(CostCard, { peak, now, key: "cost" }),
