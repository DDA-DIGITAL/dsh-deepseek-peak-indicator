...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// --- token rates --------------------------------------------------
		// V4.1 Flash official rates per the notice effective 2026-09-10 12:00
		// Beijing: per 1M tokens — cache-hit input 0.02, input (miss) 1, output
		// 4 CNY off-peak; peak is exactly 2×; weekends are off-peak all day.
		// DeepSeek had not published the USD list at build time, so USD is an
		// approximation (CNY ÷ USD_PER_YUAN) until the official list lands —
		// then replace the derived values below.
		...
		/** ¥ stays as DeepSeek prints it (≤2 decimals); $ is derived (3 decimals). */
		function ratePair(usd) {
			...
			return COST_ROWS.map((row) => ({ ...row, ...pick(row) }));
		}

		/** Token-cost card: three billing rows, off-peak vs peak, with the
		 * currently active column emphasized and a ¥/$ switch. */
		function CostCard({ peak }) {
			const [usd, setUsd] = react.useState(false);
			const rows = ratePair(usd);
			...
			return react.createElement("div", {
				...
				react.createElement("div", { className: "dpk-cost-head", key: "head" }, [
					react.createElement("span", { className: "dpk-cost-title" }, `Token cost · ${usd ? "USD" : "CNY"} per 1M`),
					react.createElement("div", { className: "dpk-cost-toggle", role: "group", "aria-label": "Rate currency" }, [
						react.createElement("button", {
							type: "button",
							className: usd ? "dpk-seg" : "dpk-seg dpk-seg-on",
							"aria-pressed": !usd,
							onClick: (event) => {
						...
						react.createElement("button", {
							type: "button",
							className: usd ? "dpk-seg dpk-seg-on" : "dpk-seg",
							"aria-pressed": usd,
							onClick: (event) => {
								event.stopPropagation();
								setUsd(true);
				...
				react.createElement("div", { className: "dpk-cost-cols", key: "cols" }, [
					react.createElement("span", { className: "dpk-cost-colgap" }),
				...
				rows.map((row) => react.createElement("div", {
					...
					react.createElement("span", { className: column === "off" ? "dpk-cost-cell dpk-cost-now" : "dpk-cost-cell" }, formatAmount(row.off, row.dec)),
					react.createElement("span", { className: column === "peak" ? "dpk-cost-cell dpk-cost-now" : "dpk-cost-cell" }, formatAmount(row.peak, row.dec))
				]))
			]);
		}
		...
		function Dashboard({ peak, now, onClose, caretLeft }) {
			...
			return [
				...
				react.createElement(CostCard, { peak, key: "cost" }),
